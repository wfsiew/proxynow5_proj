...
def insertIntoTable(tablename,record):
    ...
    if(tablename=='Request_destinations_by_MIME'):
        ...
        tmprec=record[1:3]
        curs.execute("select * from Request_destinations_by_MIME where destination=? and ContentType=? and date(InsertDate) between date('now') and date('now')",tmprec)
        ...
        if returnedQ is not None:
            try:
                ...
                curs.execute("update Request_destinations_by_MIME set request=?,KByte=?,DurationInSec=? where destination=? and ContentType=? and date(InsertDate) between date('now') and date('now')",[newrequest,newKByte,newDuration,record[1],record[2]])
            except(Exception):
                pass
        else:
        ...
        tmprec=record[1:3]
        curs.execute("select * from Requested_content_type_by_host where host=? and ContentType=? and date(InsertDate) between date('now') and date('now')",tmprec)
        returnedQ=curs.fetchone()
        ...
        if returnedQ is not None:
            try:
                ...
                newKByte=float(Kbyte)+float(record[4])
                newDuration=float(Duration)+float(record[5])
                curs.execute("update Requested_content_type_by_host set request=?,KByte=?,DurationInSec=? where host=? and ContentType=? and date(InsertDate) between date('now') and date('now')",[newrequest,newKByte,newDuration,record[1],record[2]])
            except(Exception):
                pass
        else:
        ...
        record.insert(0,datetime.datetime.now())
        tmprec=record[1:2]
        curs.execute("select * from Requested_content_type where ContentType=? and date(InsertDate) between date('now') and date('now')",tmprec)
        returnedQ=curs.fetchone()
        ...
        if returnedQ is not None:
            try:
                ...
                curs.execute("update Requested_content_type set request=?,KByte=? where ContentType=? and date(InsertDate) between date('now') and date('now')",[newrequest,newKByte,record[1]])
            except(Exception):
                pass
        else:
